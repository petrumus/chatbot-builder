...
serve(async (req) => {
  ...
  if (!N8N_WEBHOOK_URL || !N8N_AUTH_USER || !N8N_AUTH_KEY) {
    return new Response(
  ...
  if (!body.website || !body.description || !body.chatbotName) {
    return new Response(
      JSON.stringify({ error: "Missing required fields: website, description, chatbotName" }),
      {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to reach the chatbot builder service" }),
      {
        status: 502,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      }
